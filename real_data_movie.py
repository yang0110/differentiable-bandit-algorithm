...
for l in range(loop):
	...
	exp3_model=EXP3(dimension, iteration, item_num, user_feature, item_features, true_payoffs, alpha, sigma, exp3_gamma, eta)
	...
	exp3_regret, exp3_error, exp3_prob=exp3_model.run()
	...
	exp3_regret_matrix[l]=exp3_regret
	exp3_prob_matrix[l]=exp3_prob[best_arm]
...
exp3_mean=np.mean(exp3_regret_matrix, axis=0)
exp3_std=exp3_regret_matrix.std(0)
...
plt.tight_layout()
plt.savefig(path+'movielens_expucb_d_%s_item_%s'%(dimension, item_num)+'.png', dpi=300)
plt.show()
